...
fn main() -> io::Result<()> {
    ...
    println!(
        "Searching for '### Example' headings in markdown files from: {:?}",
        current_dir
    );
...
fn process_directory(dir_path: &Path, total_count: &mut usize) -> io::Result<()> {
    for entry in fs::read_dir(dir_path)? {
        ...
        if path.is_dir() {
            ...
            if let Ok(count) = count_example_headings_in_file(&path) {
                *total_count += count;
                println!(
                    "  Processed {:?}: Found {} matching headings",
                    path, count
                );
            } else {
...
fn count_example_headings_in_file(file_path: &Path) -> io::Result<usize> {
    let mut file = fs::File::open(file_path)?;
    let mut contents = String::new();
    file.read_to_string(&mut contents)?;

    // Match literal `### Example` headings only.
