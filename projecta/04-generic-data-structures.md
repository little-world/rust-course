...
- Stack<T, const N: usize> using MaybeUninit<T> for uninitialized memory
- Methods: new(), push(), pop(), peek(), len(), is_empty(), is_full()
...
- Container<T> trait with Generic Associated Types (GATs)
- Associated type Iter<'a> for iterators
...
- Methods: new(), with_default(), with_defaults(), ready(), build(),
  build_with()
...
The implementation runs successfully, producing correct output for all
data structures and passing all tests!
